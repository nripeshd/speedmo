...
function getQntdate(quantity) {
    ...
    var y = newdate.getFullYear();
       console.log(tt);console.log(date);console.log(newdate);                                              
        ...
        $.ajax({
          ...
          success: function(data) {
           ...
           error: function(x, t, m) {
              $(parent).find('.rv-box-' + index ).remove();
            /*console.log(x);
            console.log(t);
            console.log(m);*/
           },
      ...
      if ($(parent).find('.rv-main').hasClass('js-rv-slider')){
        if (displayProducts.length <= productLimit) {
          $('.js-rv-slider ').each(function(){
            $(this).find('.gallery-cell').eq(displayProducts.length).nextAll().addBack().remove();
          });
          
        } else {
          $('.js-rv-slider ').each(function(){
            $(this).find('.gallery-cell').eq(productLimit).nextAll().addBack().remove();
          });
  ...
  init: function(){
    ...
    $('.homepage-video').each(function (index, value) {
      if ($('.homepage-video').length && supports_video()) {
        ...
        if (is_touch_device() && $(window).width() <= 749 && settings.posterImage === false || $(window).width() <= 749 && settings.posterImage === false) {
          //add video element on small screens if there is no poster image
          videoSection.appendFrame($videoElement, settings);
        } else if (settings.autoPlayVideo && $(window).width() > 749){
...
var cart = {
  init: function(){
    ...
    /*if ($('#cart_form').length) {
      var params = [];
      $("[data-id].row-product").each(function(){
        params.push($(this).data("id"));
      });
      $.ajax({
        url: "https://api.maplenest.com/api/v1/products",
        data: {id: params.join(",")},
        dataType: "jsonp"
      })
      .done(function(response){
        ...
        response.products.forEach(function(data){
          var $elem = $("[data-id=" + data["id"] + "].row-product");
          $elem.find(".shipping-title b").text(data["ships_in_title"]);
          $elem.find(".shipping-description").text(data["ships_in_description"]);
        });
      });
    } */
  }
...
/*============================================================================
  Product page
==============================================================================*/
window.displayOutOfStock = function(variant){
  var productId = $('.product_form').first().data('product-id');
  if (!productId) {
    return false;
  }
  var $product = $('.product-' + productId);
  var $notify_form = $('.notify-form-' + productId);
...
var productPage = {
  init: function(){
    ...
    if($(window).width() < 767 && $('.similar-items').length > 0) {
      $('.similar-items .slides').flickity({
      "cellAlign": "left",
      "contain": true,
      "pageDots": false,
      //"groupCells": 3,
    ...
    if($('.product_gallery_nav .gallery-cell').length > 1){
  ...
  retrieveInventoryInfo: function(variantId){
    variantId = (typeof variantId !== 'undefined') ?  variantId : $("h1[data-variant-id]").data('variant-id');
    /*if ($('.js-ships-in-message').length) {
      var params = [variantId];
      $.ajax({
        url: "https://api.maplenest.com/api/v1/products",
        data: {id: params.join(",")},
        dataType: "jsonp"
      ...
      .done(function(response){
        response.products.forEach(function(data){
          $(".js-ships-in-message").text(data["ships_in_summary"]);
  ...
  initializeReview: function(){
    if (Shopify.queryParams['create-review']) {
      setTimeout(function(){ 
...
var newsletter_popup = {
  ...
  open: function(){
    if ({{ settings.newsletter_popup_mobile }} || $(window).width() > 749) {
      setTimeout( function() {
...
//     }
    ...
    if (is_touch_device() || $(window).width() < 749) {
  ...
  createAccordion: function(container, tab, content){
    ...
    $('body').on('click', specificTab, function(e){
      e.preventDefault();
        ...
        if(! $(".accordion").closest(".product-description-accordian").length ) {
           $(specificTab).not($(this)).removeClass('active');
...
//         $('ul.tabs > li > a.active').removeClass('active');
        $(this).parents('.tabs-main').find('a.active').removeClass('active');
...
$(function() {
  ...
  productPage.init();
  productPage.retrieveInventoryInfo();
  productPage.initializeReview();
  ...
  $('body').on('opened', '.js-remodal-lightbox', function () {
    ...
    if (activate_zoom && $(window).width() < 749) {
      draggable = false;
    } else {
  ...
  utils.createAccordion('.toggle-all--true', 'h5.toggle', 'ul.toggle_list');
  utils.createAccordion('.footer_menu', 'h6', 'ul');
  utils.createAccordion('.footer_content', 'h6', 'div.toggle_content');
  utils.createAccordion('.accordion', 'h5.accordion', 'div.panel');
  ...
  if ($(window).width() < 749) {
    utils.createAccordion('.toggle-all--false', 'h5.toggle', 'ul.toggle_list');
  }
  $('.toggle-all--true h5.toggle').addClass('active');
  $('.toggle-all--true ul.toggle_list').css('display','block');
  ...
  var quickShop = {
    init: function(){
      ...
      $('body').on('opened', '.js-quick-shop', function () {
        ...
        var flkty = new Flickity('.js-gallery-carousel');
        flkty.destroy();
        if($('.js-gallery-carousel .gallery-cell').length > 4){
    ...
    updateVariant: function(variant){
      ...
      if (globalQuickShopProduct != 'undefined'){
        ...
        var pro_vendor = module1.handleize(product.vendor);
        $('.js-product-vendor').html('<a href="/collections/' + pro_vendor +'">by ' + product.vendor + '</a>');
        //Product Description
    ...
    populateGallery: function(product){
      ...
      function addMainGalleryImages(multiImage){
        ...
        $.each( product.images, function( i, imgPath ){
          ...
          var $cellElems = $('<div class="gallery-cell"><div class="image__container">'+ cellContent +'</div></div>');
      ...
      function addCarouselGalleryImages(){
        $.each( product.images, function( i, imgPath ){
          ...
          var $carouselCellElems = $('<div class="gallery-cell"><div class="image__container">'+ img +'</div></div>');
  ...
  function refreshCart(cart) {
    ...
    if (cart.item_count == 0) {
      ...
      cart_action_html += '<span class="right"><span class="money">' + Shopify.formatMoney(cart.total_price, $cart.data('money-format')) + '</span></span>' +
        '<span class="right" style="margin-right:10px;">{{ 'layout.general.subtotal' | t }}:</span>';
      ...
      if(total_savings > 0 ) {
        cart_savings_html = '<span class="right"><span class="money">' + Shopify.formatMoney(total_savings - discounted_price_total, $cart.data('money-format')) + '</span></span>' +
          '<span class="right" style="margin-right:10px;">{{ 'layout.general.savings' | t }}:</span>';
      } else {
        cart_savings_html = "";
      }
    }
...
//                 setTimeout(function(){
//                 	$('#header .cart_container').removeClass('active_link');
//                 },3000);
//               } else if ($('.sticky_nav--stick').length) {
//                 $('.sticky_nav .cart_container').addClass('active_link');
//                 setTimeout(function(){
//                 	$('.sticky_nav .cart_container').removeClass('active_link');
//                 },3000);
//               } else {
...
//                 },3000);
//               }
              $('.account_menu .cart_container').addClass('active_link');
                setTimeout(function(){
                	$('.account_menu .cart_container').removeClass('active_link');
                },5000);
              $('html, body').animate({ scrollTop: $('#shopify-section-header').position().top },500);
            }
          });
        },
  ...
  $('.colle_add_to_cart').click(function(e){
    e.preventDefault();
...
var module1 = {
 ...
 if ($(window).width() <= 749) {
    $('.toogle-div-sidebar').hide();
$('.side-nav').click(function(){
// alert("drt");
  if( $('.toogle-div-sidebar').hasClass('open') ){ 
    
   $('.toogle-div-sidebar').removeClass('open').fadeOut();
    $(this).removeClass('open');
  }
  else{
  
  
    $('.toogle-div-sidebar').addClass('open').fadeIn();
     $(this).addClass('open');
  }
...
$(document).ready(function(){
  ...
  $(".tab_drawer_heading").click(function() {
    var $parent = $(this).parents('.tabs_combine');
    if(!$(this).hasClass('d_active')){
    $parent.find(".tab_content").slideUp();
    var d_activeTab = $(this).attr("rel"); 
    $parent.find("#"+d_activeTab).slideDown();
    ...
    $('html, body').animate({ scrollTop: $parent.find('.tabs-content').position().top -100},500);
  });
});

$(document).ready(function(){
  $('.more_by_vendor .products-slider').flickity({
    "lazyLoad": 4,
    "imagesLoaded": true,
    "draggable": true,
    "cellAlign": $(window).width() <= 749 ? "center":"left",
    "prevNextButtons": true,
    "wrapAround": true,
    "pageDots": false,
    "contain": true,
...
$(document).ready(function(){
  $('.faq_header').click(function(){
    var $this = $(this).parents('.faq_questions');
    if($this.find('.faq_text').css('display') == 'none'){
      $this.find('.faq_header').addClass('active');
    }else{
      $this.find('.faq_header').removeClass('active');
    } 
    $this.find('.faq_text').slideToggle();
  });
  var c_url = $(location).attr('href');
  if (c_url.indexOf("faqs#") >= 0){
    var arr_c_url = c_url.split('faqs#');
    $('html, body').animate({ scrollTop: $('#'+arr_c_url[1]).position().top + 40 },500);
  }
});
  $('.faq_menu a').click(function(){
    var $this = $(this).data('id');
    $('html, body').animate({ scrollTop: $('#'+$this).position().top + 40 },500);
    $('.faq_menu li a').removeClass("active");
    $(this).addClass("active");
    return false;
  });
    if($('#faq_nav').length > 0){
      $(window).scroll(function(){
        ...
        var div_top = $('#faq_nav').offset().top;
        if (window_top >= div_top) {
          $('#faq_nav nav').addClass('fixed');
          $('#faq_nav').css('height',$('#faq_nav nav').height());
          $('.menu-company').css('top',$('#faq_nav nav').height());
        } else {
          $('#faq_nav nav').removeClass('fixed');
          $('#faq_nav').css('height','');
          $('.menu-company').css('top','');
        }
      });
  ...
  function onScroll(event){
    ...
    $('.faq_menu a').each(function () {
      ...
      if( scrollPos > 700 ){
        var header_top = $('.main_nav_wrapper').height();
        $('#faq_nav nav').css('top', header_top );
      }else{
        $('#faq_nav nav').css('top', '' );
      }
  ...
  $(document).ready(function(){
      if($(window).width() > 749){
        if($('.cartlistitem .item').length > 2){
          $('.cartlistitem').flickity({
            // options
            "lazyLoad": 2,
            "pageDots": false,
            "groupCells": 1, 
